...
export default function GuidePage() {
  ...
  const handlePrint = () => {
    const guideTitle = guide?.sermonTitle || guide?.title || 'Roteiro';
    const originalTitle = document.title;
    document.title = guideTitle;
    window.scrollTo(0, 0);
    setTimeout(() => {
      window.print();
      // Restaura o título original após o diálogo de impressão fechar
  ...
  const handleSaveNotes = async () => {
    ...
    setSavingNotes(true);
    try {
      await fetch(`/api/mdc/meetings/${meetingId}/guide`, {
        method: 'POST',
  ...
  const offlineKey = meetingId ? `emetis-guide-${meetingId}` : null;
  ...
  const handleGenerate = async () => {
    ...
    setGenError('');
    try {
      const res = await fetch('/api/mdc/ai/generate-guide', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleSave = async (publish = false) => {
    setSaving(true);
    const res = await fetch(`/api/mdc/meetings/${meetingId}/guide`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  if (guide && !editing) {
    ...
    return (
      <div className="h-full overflow-y-auto bg-gray-100 pb-24 print-root">
        ...
        <style>{`
          @media print {
            /* Remove todo overflow/height para o conteúdo fluir completo */
            html, body {
            ...
            /* Expande todos os containers com scroll */
            body > *, #__next, [data-nextjs-scroll-focus-boundary] {
              height: auto !important;
              overflow: visible !important;
            }
            ...
            /* Expande o container raiz que tem h-full overflow-y-auto */
            .print-root {
              height: auto !important;
              overflow: visible !important;
            ...
            .print-content .bg-indigo-50 { background: #eef2ff !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            .print-content .bg-emerald-50 { background: #ecfdf5 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            /* Evita cortes no meio de seções */
    ...
    <div className="h-full overflow-y-auto bg-gray-50 pb-24">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 space-y-3">
          <div>
            <p className="text-sm font-bold text-indigo-900">Teo</p>
          ...
          <button onClick={handleGenerate} disabled={generating || (!form.biblePassage && !form.sermonContent)}
            ...
            {generating ? 'Gerando roteiro...' : 'Gerar Roteiro com Teo'}
          </button>
          {genError && (
