...
export default function NewMeetingPage() {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/mdc/meetings', {
      method: 'POST',
    ...
    if (res.ok) {
      router.push(`/mdc/cells/${cellId}/meeting/${data.id}/guide`);
    } else {
    ...
    <div className="h-full overflow-y-auto bg-gray-50 pb-24">
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          <Link href={`/mdc/cells/${cellId}`} className="text-gray-500 hover:text-gray-700">←</Link>
