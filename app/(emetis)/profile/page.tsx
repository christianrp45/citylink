...
import { LogOut, Edit2, MapPin, Phone, Mail, Users, Camera, Loader2, Bell, BellOff, Check, X, Shield, ChevronDown, ChevronUp, ChevronRight, Download, Trash2, Clock, CheckCircle, Heart, Home, Briefcase, Church, Plus, QrCode, Copy, Calendar, Eye } from 'lucide-react';
...
import { LevelBadge, GlyphFlame } from '@/components/emetis-icons/missions';
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 pb-24">
      ...
      <div className="px-4 mt-4 space-y-4">
        ...
        {/* Nível & Missões — resumo compacto; detalhes completos e conquistas em /missions */}
        {missions && (() => {
          const LEVEL_STYLES: Record<string, { bg: string; text: string; border: string }> = {
            semente: { bg: 'bg-slate-50', text: 'text-slate-700', border: 'border-slate-200' },
            broto:   { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200' },
            árvore:  { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200' },
            fruto:   { bg: 'bg-violet-50', text: 'text-violet-700', border: 'border-violet-200' },
            luz:     { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200' },
          };
          ...
          return (
            <Link href="/missions" className={`block rounded-2xl border p-4 ${style.bg} ${style.border}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{missions.total} XP · {missions.weekPoints} pts esta semana</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {missions.currentStreak > 0 && (
                    ...
                    >
                      <GlyphFlame size={12} /> {missions.currentStreak}
                    </span>
                  )}
                  <ChevronRight size={18} className="text-slate-300" />
                </div>
              </div>
              <p className="text-xs text-indigo-600 font-semibold mt-2">Ver missões e conquistas →</p>
            </Link>
          );
        })()}
