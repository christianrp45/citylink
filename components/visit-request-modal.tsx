...
export default function VisitRequestModal({ user, onClose }: Props) {
  ...
  const handleSend = () => {
    setStatus('pending');
    // Simula resposta automática se mesa posta
    if (user.openToVisits) {
        ...
        <div className="bg-white rounded-2xl w-full max-w-md">
          <div className="p-6 text-center">
            ...
            <p className="text-slate-500">
              {user.openToVisits ? `${user.name} aceita visitas livremente. Pode ir!` : `${user.name} aceitou sua visita!`}
            </p>
            {user.homeLocation && (
            ...
            <button onClick={onClose} className="mt-6 w-full py-3 rounded-xl bg-slate-100 text-slate-700 font-semibold hover:bg-slate-200 transition-colors">
              Fechar
            </button>
          </div>
        </div>
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-md">
          <div className="p-6 text-center">
            ...
            <button onClick={onClose} className="mt-6 w-full py-3 rounded-xl bg-slate-100 text-slate-700 font-semibold">Fechar</button>
          </div>
        </div>
      </div>
    );
